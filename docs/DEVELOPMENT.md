...
Requirements: Node `20` (see `.node-version`) and npm.
...
- Added an RSS feed at **`/rss.xml`** via `gatsby-plugin-feed` (`^4.25.0`).
  One combined feed covering both blog and homelab posts (`template` in
  `[blog-post, homelab-post]`), newest first.
- Feed config lives in `gatsby-config.js`; it reads the site
  title/description/`siteUrl` from `src/util/site.json`.
- Added an RSS icon (`src/components/rss-link.js`) in the header, just **before
  the search icon** (both the mobile and desktop placements), linking to
  `/rss.xml`.
- Note: the feed file is only generated by a **production build**
  (`gatsby build` / Netlify), not by `gatsby develop`.

### 2026-06-23 — Build fix: empty tag collided with /tags/

- A post had an empty tag (`tags: [..., ""]`). An empty value slugifies to
  nothing, so `gatsby-node` generated a `/tags//` page that normalized to
  `/tags/` and collided with the redirect page — failing the build with
  "Query was not run". `gatsby-node` now **skips empty/blank tag & category
  values**, and the stray empty tag was removed from the post.
